  ...
  const CreateChooser = () => {
    return (
      <div className="space-y-4">
        ...
        <div className="flex justify-end gap-2">
          <button type="button" onClick={() => setMode('select')} className="px-4 py-2 rounded-lg border hover:bg-gray-50">
            Back
          </button>
        </div>
      </div>
                  ...
                  filtered.map((a) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-lg border hover:bg-gray-50 ${
                        String(a.id) === String(value) ? 'bg-gray-50 border-gray-300' : 'border-gray-200'
                      }`}
                    >
                      <div className="text-sm font-medium text-gray-900">{a.code ? `${a.code} - ` : ''}{a.name}</div>
                      {(a.type || a.subType || a.ledgerCategory) && (
                        <div className="text-xs text-gray-500 truncate">{[a.ledgerCategory, a.type, a.subType].filter(Boolean).join(' • ')}</div>
                      )}
                    </button>
                  ))
                )}
